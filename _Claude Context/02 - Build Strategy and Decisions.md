...
`make_edit_docs.py`, `read_edit_doc.py`, `make_informed_shell.py`,
`verify_refs.py` and `md2docx.py`. Tracked in git; the last three were added
...
regenerates everything and will silently destroy tracked changes on any module
Neal has reviewed but not yet handed back.

Its module list now holds twenty-nine entries: twelve Novice (N1 to N8 plus the
four N7 topics) and all seventeen Informed, added 2026-08-05. `build()` skips any
page that does not exist, so listing a module before it is written is harmless.
The two Novice offshoot docs, `N2-Offshoot-BedsideDO2VO2` and
`N5-Offshoot-RAPVolume`, are still not in the list and cannot be regenerated.
Both offshoot pages were deleted during the review pass, so the docs are kept only
as the record of that instruction.

**Per-module issue blocks, added 2026-08-16.** `make_edit_docs.py` reads an
optional file per module from `Module Edit Docs/_issues/<LABEL>.md` and renders it
as a coloured block at the top of that module's doc, so a module's known problems
travel with the prose they concern instead of living in a separate document. Each
line is classed `SOURCE:`, `DECISION:` or `NOTE:`. No file means no block, which
is what makes it safe on the Novice docs. The convention is documented in
`Module Edit Docs/_issues/README.md`.

**`make_informed_shell.py` generates the Informed navigation. Never hand-edit a
stepper or a pager.** Two destructive regex bugs were fixed in it on 2026-08-15;
before that, a chrome refresh would have deleted the reference list from every
page it touched. The assertions now in `refresh()` exist because the failure was
silent.

**`verify_refs.py` is the central references pass** and file 04 requires it be run
once per batch rather than per module. Its header records two NCBI gotchas worth
not rediscovering.
