...
The first saves a snapshot on this Mac. The second copies it to GitHub.

Both are run from this Hemosim folder. Claude Code can run them for you — just say what you changed.

Write commit messages describing **why** something changed, not what file it was. Git already knows which files changed. "Rewrote N4 waveform section after Dr. Smith's feedback" is useful in a year. "Updated file" is not.
...
**Push** — upload your commits to GitHub. Manual. Nothing goes to GitHub until you push.
...
Your version history is on **this Mac**, in the hidden `.git` folder inside the Hemosim folder. That's the original. It works with no internet.

GitHub holds a **copy** of it, updated only when you push. It is an off-site backup and a way to browse things in a web browser. If you commit for three weeks without pushing, all three weeks are safe locally — GitHub just shows the older state until you push.

---

## The two separate repos

**`nchaisso/hemosim-content`** — this repo. The course documents. Created July 28, 2026. Lives in Dropbox at `Dropbox/Claude/CCM/Fellowship/Hemosim`.

**`nchaisso/hemosim-web`** — the code that runs the live website. Created July 24, 2026. Its working copy on this Mac has not been located; only its presence on GitHub is confirmed. *(Open question — worth pinning down before the next website change.)*

They are separate because a repo tracks one folder, and these are two different folders holding two different kinds of work. Keeping them apart means each has a clean, readable history instead of website changes and document edits interleaved.
...
| Location | Contents |
|---|---|
| `Module Edit Docs/` | 14 Word files — N1–N8, N7 topic breakouts, offshoots |
| `Hemosim COntent Outlines for Wix Build/` | 8 Word files — content outline v1–v8 |
...
- `.DS_Store` and `~$*` — macOS and Word junk files
...
There are currently eight files named `HemoSim Content Outline by Expertise Level` through `(v8)`. Git will preserve all eight forever — it sees eight unrelated documents.
...
**Decision made July 28, 2026:** deferred. Keep v1–v8, build the commit habit first, revisit later. Clutter costs nothing.

---

## Notes and open items

- **One computer.** Git and Dropbox both version files and do not coordinate. On a single machine this is fine. If a second Mac ever enters the picture, revisit this — Dropbox syncing the `.git` folder mid-operation across two machines can corrupt it.
- **Locate the `hemosim-web` working copy** on this Mac.
- **Standing preference:** Claude should confirm which folders to include before each commit, since what matters may change over time.
